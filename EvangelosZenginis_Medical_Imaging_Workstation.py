...
def crop_nonblack_area(im, threshold=15, padding=8):
    arr = to_uint8_image(im)

    # Κρατάει μόνο αρκετά φωτεινά pixels ώστε να αγνοεί μαύρο φόντο/artefacts
    mask = arr > threshold
...
def pad_to_same_canvas(im1, im2, fill=0):
    """
    Βάζει δύο εικόνες σε ίδιο canvas χωρίς να τις τεντώνει.
    Έτσι δεν μεγαλώνει/παραμορφώνεται καμία εικόνα.
    """
    a = to_uint8_image(im1)
    b = to_uint8_image(im2)

    h = max(a.shape[0], b.shape[0])
    w = max(a.shape[1], b.shape[1])

    canvas_a = np.full((h, w), fill, dtype=np.uint8)
    canvas_b = np.full((h, w), fill, dtype=np.uint8)
    ...
    canvas_a[ya:ya + a.shape[0], xa:xa + a.shape[1]] = a
    canvas_b[yb:yb + b.shape[0], xb:xb + b.shape[1]] = b

    return canvas_a, canvas_b

def prepare_same_visual_size(original, processed, threshold=10, padding=15):
    """
    Κάνει crop στο μαύρο φόντο, αλλά ΔΕΝ τεντώνει τις εικόνες.
    Μετά τις βάζει σε ίδιο canvas ώστε να εμφανίζονται ίδιου μεγέθους στο Streamlit/PDF/slider.
    """
    original_crop = crop_nonblack_area(original, threshold=threshold, padding=padding)
    processed_crop = crop_nonblack_area(processed, threshold=threshold, padding=padding)

    original_same, processed_same = pad_to_same_canvas(original_crop, processed_crop)

    return original_same, processed_same

def make_same_display_pair(original, processed, threshold=15, padding=8):
    """
    Κάνει crop και στην αρχική και στην reconstructed εικόνα,
    ώστε να αφαιρεθεί το μαύρο πλαίσιο και μετά τις φέρνει στο ίδιο μέγεθος.
    """
    original_crop = crop_nonblack_area(original, threshold=threshold, padding=padding)
    processed_crop = crop_nonblack_area(processed, threshold=threshold, padding=padding)

    processed_crop = resize_like(processed_crop, original_crop)

    return original_crop, processed_crop
...
# Crop για να φύγει το μαύρο περιθώριο της ανακατασκευής
        # Κρατάμε ξεχωριστή έκδοση για σωστή σύγκριση ίδιου οπτικού μεγέθους
        display_input4, processed4_raw = prepare_same_visual_size(
            active_im,
            processed4_raw,
            threshold=10,
            padding=20
        ...
        st.session_state["chapter4_display_input"] = display_input4
        ...
        record_output("Chapter 4 result", processed4, f"Tomography {algorithm}", params4,   input_image=display_input4,  input_source=st.session_state.get("pipeline_source", "Original / noisy input"))
    ...
    with view:
        if "tomo_result" in st.session_state:
            ...
            c1, c2 = st.columns(2)
            display_input4 = st.session_state.get("chapter4_display_input", active_im)


            c1.image(
                 to_uint8_image(display_input4),
